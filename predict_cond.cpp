...
int main(int argc, char * argv[]) {
    ...
    const string trainfile = "data_full/all.dta.bin.zeroProbe";
    ...
    // Scan the h5file to determine the number of features
    size_t N_feature;
    sscanf(h5file.c_str(), "states_cond_%zufeat_&[\n]", &N_feature);
    ...
    for (vector<float>::iterator it = predictedRatings.begin(); 
        ...
        fout.write((char *) &(*it), sizeof(float));
    }
    fout.close();
